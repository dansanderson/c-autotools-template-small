...
4. In `Makefile.am`, create a new `tests_runners_test_modname_SOURCES`
   definition where `modname` is the name of the module. Its value consists of:
    1. A test runner source file that Unity Test will generate automatically
    2. The module test source file
    3. The module's source and header files
    4. Mock source files (`tests/mocks/mock_xxx.c` and `.h`) for other modules
...
`autoreconf`. (See my
[superclean.py](https://github.com/dansanderson/c-autotools-template/blob/main/scripts/superclean.py)
script from the other template if this is bothersome.)
